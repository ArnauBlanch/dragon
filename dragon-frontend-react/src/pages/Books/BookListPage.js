...
const columns = t => [
    ...
    { title: t('books.author'), dataIndex: 'author', searchable: true },
    {
        ...
        render: (availableCopies, book) => (
            <Progress
                type="line"
                status={availableCopies === 0 ? 'exception'
                    : availableCopies === book.totalCopies ? 'success': 'normal'}
    ...
    },
    {
        title: t('books.price'),
        dataIndex: 'price',
        render: price => `${t('decimals-with-comma') ? price.toFixed(2).replace('.', ',') : price.toFixed(2)} €`,
